...
function render_project(
  ...
  img.src = figure_path;
  img.setAttribute("style", "width: 100%; max-height: 100px; object-fit: cover;");
  ...
  for (var idx = 0; idx < author_list.length; idx++) {
    if (idx < author_list.length - 1 && (author_list[idx + 1] == "equal" || author_list[idx + 1] == "corresponding")) {
      author.innerHTML += '<span name="' + author_list[idx] + '" title="' + author_list[idx + 1] + '"></span>';
      idx++;
    } else {
      author.innerHTML += '<span name="' + author_list[idx] + '" title="' + '"></span>';
    }
  ...
  for (var idx = 0; idx < material_list.length; idx++) {
    material.innerHTML += '<a href="' + material_list[idx][1] + '" target="_blank">' + material_list[idx][0] + "</a>";
...
function render_author(author_name, link = null, alias = null) {
  ...
  for (var idx = 0; idx < list.length; idx++) {
    ...
    if (link) {
      list[idx].innerHTML = '<a href="' + link + '" target="_blank">' + context + "</a>";
    } else {
...
render_project(
  ...
  (author_list = ["Yuze He", "Chen Bian", "Jingfei Xia", "Shuyao Shi", "Zhenyu Yan", "Qun Song", "Guoliang Xing"]),
  (material_list = [
  ...
  (description_content =
    "VI-Map is the first system that leverages roadside infrastructure to enhance real-time HD map construction for autonomous driving.")
);
...
render_project(
  ...
  (author_list = ["Yuze He", "Li Ma", "Jiahe Cui", "Zhenyu Yan", "Guoliang Xing", "Sen Wang", "Qintao Hu", "Chen Pan"]),
  (material_list = [
    ["SenSys 2022", "https://dl.acm.org/doi/abs/10.1145/3560905.3568519"],
    [
      "Slides",
      "https://docs.google.com/presentation/d/1jHNdxwvrZgbOQABaElAjVCxM9QP9ExZq/edit?usp=sharing&ouid=105439115813761246402&rtpof=true&sd=true",
    ],
  ]),
  (award_name1 = null),
  (award_name2 = null),
...
render_project(
  ...
  (author_list = ["Yuze He", "Li Ma", "Zhehao Jiang", "Yi Tang", "Guoliang Xing"]),
  (material_list = [
    ["MobiCom 2021", "https://dl.acm.org/doi/10.1145/3447993.3483276"],
    ["Teaser", "https://www.youtube.com/watch?v=IYu8d8Rzy2s"],
    [
      "Slides",
      "https://docs.google.com/presentation/d/1XvCcihCw_QwEhLiFPr62qV-a2tN3rzXj/edit?usp=sharing&ouid=105439115813761246402&rtpof=true&sd=true",
    ],
  ]),
  (award1_name = null),
  (award2_name = null),
  (description_content =
    "VI-Eye combines the infrastructure LiDAR point cloud with the autonomous vehicle's LiDAR point cloud, fundamentally boosting vehicle's 3D perception.")
);
